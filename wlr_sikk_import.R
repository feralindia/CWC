...
for(i in 1:length(num_wlr)){
    ...
    wlrtab.cal<-paste("wlr_", num_wlr[i],"_calib", sep="") ## use "_calib" for agn without for nlg
...
## FIX HERE  THE TIME SERIES COMMAND BELOW FLIPS THE DATE_TIME TO GMT
    ...
    ## mmx$date_time <- as.POSIXct(mmx$date_time, tz="GMT")##, usetz=TRUE
    mmx$date_time <- as.POSIXct(mmx$date_time, tz="Asia/Kolkata", usetz=TRUE)
    mmx$date_time <- mmx$date_time + 19800 ## not required
    ## THE ADDITION OF 5Hrs 30Mins DONE MANUALLY
    ##mmx <- mmxall[-1,]
    ## attributes(mmx$date_time)$tzone <- "Asia/Kolkata"

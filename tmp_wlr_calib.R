...
wlr_calib_list <- list.files(path=dir_calib_wlr, pattern="csv")
col_names <- c("y", "x")
## ## Create table to hold calibration results. --commented out to remove SQL
## stm <- paste("DROP TABLE IF EXISTS ", site, "wlr_calib; 
## CREATE TABLE ", site, "wlr_calib
## (id SERIAL PRIMARY KEY, wlr_id TEXT, intercept REAL, x REAL);", sep="")
## rs <- dbSendQuery(con, stm)
##   rm(stm)
setwd(rdata.dr) # set the wd
...
## ## Transfer the calibration data to the database table --- Commented out to remove SQL
## stm<-paste("COPY ", site, "wlr_calib (wlr_id, intercept, x) FROM '", outcoef, 
##   "' DELIMITER ',' CSV header;", sep="")
##     rs<-dbSendQuery(con,stm)
##     rm(stm)
## Use ggplot to plot a panel of the calibration
## First create a df with all the data
tmp$wlr <- wlr_nme
...
outplot <- ggplot(data = alltmp, aes(x, y)) + 
    geom_point() + facet_wrap(~wlr) +
    stat_smooth(method = "lm", col = "red") +
   # theme(axis.text.x = element_text(angle = 90))+
    labs(x="Capacitance", y="Stage in mm")

qplot(data=alltmp, x, y, facets=~wlr)+stat_smooth(method="lm")+labs(x="Capacitance", y="Stage in mm")

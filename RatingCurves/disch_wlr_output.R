...
## convert the equations into statements for each WLR - this has to be done manually
stm.wlr102 <- "SELECT wl_cal/100 AS Stage, 0.5768*(((wl_cal/100)-0.09369)^0.8627) AS Discharge FROM nilgiris.wlr_102 WHERE  wl_cal/100>0.09369" ## ensure the values are positive
stm.wlr103 <- "SELECT wl_cal/100 AS Stage, 9.8006*(((wl_cal/100)-0.04132)^2.3954) AS Discharge FROM nilgiris.wlr_103 WHERE wl_cal/100>0.04132"
stm.wlr106 <- "SELECT wl_cal/100 AS Stage, 6.3776*(((wl_cal/100)-0.005)^3.0746) AS Discharge FROM nilgiris.wlr_106 WHERE  wl_cal/100>0.005"

## pull in the results, save to csv and plot
csvdir <- "/home/udumbu/rsb/OngoingProjects/CWC/Data/Nilgiris/rating/csv/"
figdir <- "/home/udumbu/rsb/OngoingProjects/CWC/Data/Nilgiris/rating/fig/"

...
for (i in 1:length(pyg.dir)){
    ...
    for (j in 1: length(cx.flst)){
        ...
        ## are we mixing up the cross sections and velocities here?
        ...
        tmp.cxfix$start <- tmp.cxfix$start/100 # convert to metres
        tmp.cxfix$end <- tmp.cxfix$end/100 # convert to metres
        ...
        for (k in 1 : n.rect.cxfix){
            ...
            tmp.res[k, 5] <- stage
            tmp.res[k, 6] <- k
            tmp.res[k, 7] <- ar.int # removed /1000 - converted to m after import of tmp.cxfix
        ...
        res[j, 4] <- "pygmy"
        res[j, 5] <- stage
        ## Add the separate average readings for tmp.res to a single reading

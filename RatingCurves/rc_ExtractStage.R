...
ts.cxall$dt.tm<-as.POSIXct(ts.cxall$dt.tm, format="%Y-%m-%d %I:%M:%S %p", tz = "Asia/Kolkata")
    ...
    for (m in 1: length(stn.dir)){
        ...
        ##--Nov 17 added to adjust for wlr height - to be removed
        stn.csv$cal <- stn.csv$cal + .2 # remove this? needs to be checked for each WLR
        ...
        write.table(tmp.ext, file=stage.file, append=TRUE, sep = ",")  ## overwrites older file
    }
##}
...
## Nov '17
## Consider adding a value to the stage so that a higher stage (incorporating the height of the stilling well which is below the flow) is included. This will result in a corrected (lower) discharge for the stage values.

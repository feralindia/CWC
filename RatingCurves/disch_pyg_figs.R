...
for (i in 1:length(pyg.dir)){
    ...
    ## @knitr chunk 3
        ...
        ## tmp.pyg <- read.csv(file="~/tmp/01sept2014.csv", header = T, skip=5)
        ...
        for (k in 1 : nrow(tmp.pyg)){ ## changed from 1: n.rect
            rect <- as.data.frame(matrix(ncol = 2)) # Needs to be fixed from here 25Nov2014
            names(rect) <- c("x", "y")
            if(k == 1){
            ...
            plot(gpc.rect, poly.args = list(border = k + 1, lty = lnstl), add = TRUE) # uncomment to plot the segment

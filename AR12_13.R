...
repdir<- "/home/udumbu/rsb/OngoingProjects/CWC/Reports//ForestDeptNilgiris/2014/"
tbrgnum <- 101:126
tbrgname <- paste("Rain Gauge No.",101:126, sep="")
daily.files <- paste("tbrg_",101:126,"_1 day.csv", sep="")
...
allcumdat <- subset(allcumdat, subset= dt.tm > "May 2013")
...
## cumulative for 2014
allcumdat14 <- subset(allcumdat, subset= dt.tm > "Dec 2013")
cumrain14.tbrg<- aggregate(allcumdat14$mm, by=list(allcumdat14$tbrg), FUN=sum, na.rm=TRUE)
...
summaryrain <- subset(summaryrain, subset= as.numeric(summaryrain$tbrg) < 10 &  mnt > "May 2013")
summaryrain$mnt <- as.POSIXct(summaryrain$mnt, format= "%m %Y")## as.yearmon(summaryrain$mnt)
...
epsout <- paste(repdir,"TBRGAvgRain2013_14.eps", sep="")
pdfout <- paste(repdir,"TBRGAvgRain2013_14.pdf", sep="")
ggsave(avgrainplot, file=epsout, width=240, height=180, units="mm")
ggsave(avgrainplot, file=pdfout, width=240, height=180, units="mm")
...
head(cumrain14.tbrg)
cumrain2014 <- ggplot(data=cumrain14.tbrg, aes(x=Group.1, y=x)) + ## aes(x="Rain Gauge", y="Rain in mm"))+  ## aes(x=Group.1, y=x))  +
...
epsout <- paste(repdir,"CumRain2014.eps", sep="")
pdfout <- paste(repdir,"CumRain2014.pdf", sep="")

...
tbrgl <- melt(alltbrg, id.vars=c("tbrg","dt"), measurevars="mm_rain")
tbrgw <- dcast(tbrgl, tbrg+dt~variable, value.var="value")
outplot <- ggplot(data = tbrgw, aes(x=dt, y=mm_rain)) + 
...
## sumrainl <- melt(summaryrain, id.vars=c("tbrg","mnt"),  value.var=c("avg", "cum"))
## tbrgw <- dcast(tbrgl, tbrg+dt~variable, value.var="value")
fix(summaryrain) ## for some reason ggplot2 throws an error in the next graph if this is not done
